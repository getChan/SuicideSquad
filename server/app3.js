...
//데이터베이스 변수
var dbName = 'suicide';
var collectionName = 'dictionary2';
var database;
function connectDB() {
    //var inputWord = "가까이";
...
var quiz = function(database, callback){
    console.log('Quiz 호출됨');
    var index = 0;  // lable 값
    var collection = database.collection(collectionName);
    
    collection.find({"lable":index++}).toArray(function(err, docs){
        if(err){
            callback(err, null);
            return;
        }
        if(docs.length > 0){
            console.log('lable %d, 단어 %s 를 찾음 ',index, docs[index].word);
        }else {
            console.log('단어를 찾지 못함.');
            callback(null, null);
        }
    });
}
...
router.route('/process/quiz').post(function(req,res){
    console.log('/process/quiz 호출됨.');
    var check = req.body.check || req.query.check;
...
app.post('/android/word', function (req, res) {
    console.log('/android/word 호출됨.');
    var paramWord = req.body.wordName || req.query.wordName;
    console.log(paramWord + ' 검색');
    if (database) {
        searchWord(paramWord, database, function (err, docs) {
...
router.route('/process/multipart').post(upload.array('video', 1), function(req, res) {
	...
	try {
		...
		res.write('원본 파일명 : ' + originalname + ' -> 저장 파일명 : ' + filename);
		res.end();
